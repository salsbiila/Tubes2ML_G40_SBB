...
from layer_embedding import EmbeddingLayer
from layer_LSTM import LSTMLayer
from layer_bidirectionalLSTM import BidirectionalLSTMLayer
    ...
    def _build_layers(self):
        ...
        for i, layer in enumerate(self.model.layers):
            ...
            if layer_type == 'Embedding':
                ...
                self.layers.append(BidirectionalLSTMLayer(forward_weights, backward_weights, merge_mode))
                layer_idx += 1
            
            elif layer_type == 'LSTM':
                weights = layer.get_weights()
                print(f"  LSTM weights shapes: {[w.shape for w in weights]}")
                
                return_seq = getattr(layer, 'return_sequences', False)
                go_back = getattr(layer, 'go_backwards', False)
                
                self.layers.append(LSTMLayer(weights, return_sequences=return_seq, go_backwards=go_back))
                layer_idx += 1
